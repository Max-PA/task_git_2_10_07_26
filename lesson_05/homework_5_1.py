...
car_data: dict[str | Any, tuple[str, int, float, str, int] | Any] = {
...
# Порожній список для додавання даних результату пошуку
search_result = []

# Перебираємо всі автомобілі у словнику
for brand, car_info in car_data.items():
    # задаємо критерії пошуку в словнику car_data
    if (
       car_info[1] >= search_criteria[0] and
       car_info[2] >= search_criteria[1] and
       car_info[4] <= search_criteria[2]
    ):
        # додаємо результати в список search_result
        search_result.append((brand, car_info))
        # Відображаємо результати пошуку згідно критеріїв
        print(brand, car_info)

# Сортуємо результати за зростанням вартості авто
sorted_search_result = sorted(search_result, key=lambda x: x[1][4])

# Відображаємо тільки перші 5 результатів пошуку
print(sorted_search_result[:5])